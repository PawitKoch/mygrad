...
def draw_dot(root):
    ...
    for n in nodes:
        ...
        color = (
            "lightgreen" if len(n._prev) == 0 else  # Leaf/input nodes
            "orange" if n._op else 
            "lightblue"
        )
    ...
    nx.draw(dot, pos, with_labels=True, labels=labels, node_size=1800, node_color=colors, font_size=8, edgecolors='black', linewidths=0.5)
    plt.show()
